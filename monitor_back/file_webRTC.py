...
SIGNALING_SERVER = 'wss://i12e101.p.ssafy.io/ws'
...
class VideoFileTrack(VideoStreamTrack):
    ...
    def __init__(self, filename):
        ...
        if not self.cap.isOpened():
            raise RuntimeError(f"비디오 파일을 열 수 없습니다: {filename}")
    ...
    async def recv(self):
        ...
        if not ret:
            # 비디오 파일 끝에 도달하면 처음으로 돌아갑니다.
            self.cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
            ret, frame = self.cap.read()
            if not ret:
                logger.error("비디오 파일에서 프레임을 읽을 수 없습니다")
                return None

        # 프레임을 좌우 반전합니다.
        frame = cv2.flip(frame, -1)

        # OpenCV BGR 형식을 RGB로 변환합니다.
...
async def run_webrtc():
    ...
    try:
        # 비디오 파일 경로를 수정하세요.
        video_sender = VideoFileTrack("IMG_3330.MOV")
        video_track = pc.addTrack(video_sender)
        logger.info("비디오 파일 트랙이 추가되었습니다.")
